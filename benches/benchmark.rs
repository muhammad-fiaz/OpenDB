...
use opendb::{OpenDB, OpenDBOptions, Memory};
...
fn graph_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("get_related", |b| {
        ...
        for i in 0..100 {
            ...
            if i > 0 {
                db.link(&format!("mem_0"), "related", &format!("mem_{}", i)).unwrap();
            }
        }
...
criterion_group!(benches, kv_benchmarks, memory_benchmarks, vector_search_benchmarks, graph_benchmarks, transaction_benchmarks);
