...
fn main() -> Result<()> {
    println!("{}", "OpenDB Custom Storage Location Example".bright_cyan().bold());
    println!("{}", "========================================".bright_cyan());
    println!();
    
    println!("{}", "Note: OpenDB creates a folder (not a single file) with multiple files inside.".bright_magenta());
    println!("{}", "Check the OPENDB_INFO file in any database folder for details.".bright_magenta());
    println!();

    // Example 1: Default storage location
    println!("{}", "1️⃣  Default Storage Location".bright_yellow());
    let db1 = OpenDB::open("./data/default_db")?;
    println!("  {} Database folder created at: {}", "✓".green(), "./data/default_db/".bright_white());
    println!("  {} Contains: OPENDB_INFO, *.log, *.sst, MANIFEST, etc.", "📁".cyan());
    drop(db1);
    ...
    println!("  {} Database configured with custom path: {}", "✓".green(), custom_path.bright_white());
    ...
    println!("{}", "3️⃣  Environment-based Storage (Production)".bright_yellow());
    let db_path = env::var("OPENDB_PATH")
        .unwrap_or_else(|_| "./data/production_db".to_string());
    ...
    println!("  {} Production database at: {}", "✓".green(), db_path.bright_white());
    ...
    for tenant in tenants {
        ...
        let tenant_options = OpenDBOptions::with_dimension(384)
            .with_kv_cache_size(1000);
        
        let db = OpenDB::open_with_options(&tenant_path, tenant_options)?;
        println!("  {} Tenant database: {} at {}", "✓".green(), tenant.cyan(), tenant_path.bright_white());
    ...
    println!("  {} Recommended path for this platform:", "💡".bright_yellow());
    println!("    {}", platform_path.bright_white());
    
    println!();

    // Best practices summary
    println!("{}", "📚 Storage Location Best Practices:".bright_cyan().bold());
    println!();
    
    println!("{}", "  Development:".bright_white());
    println!("    {} Use relative paths like {}", "•".yellow(), "./data/dev_db".cyan());
    println!("    {} Keep databases in project directory", "•".yellow());
    println!();
    
    println!("{}", "  Production:".bright_white());
    println!("    {} Use environment variables for configuration", "•".yellow());
    println!("    {} Choose platform-specific paths:", "•".yellow());
    println!("      {} Linux: {}", "→".bright_blue(), "/var/lib/opendb".cyan());
    println!("      {} macOS: {}", "→".bright_blue(), "/usr/local/var/opendb".cyan());
    println!("      {} Windows: {}", "→".bright_blue(), "C:\\ProgramData\\OpenDB".cyan());
    println!();
    
    println!("{}", "  Multi-Tenant:".bright_white());
    println!("    {} Isolate each tenant in separate database", "•".yellow());
    println!("    {} Use pattern: {}", "•".yellow(), "./data/tenants/<tenant_id>".cyan());
    ...
    println!("{} For more info, visit: {}", "💡".bright_yellow(), "https://muhammad-fiaz.github.io/opendb".bright_blue().underline());
